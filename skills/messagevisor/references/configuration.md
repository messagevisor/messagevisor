...
Catalog message details are always written to content-addressed blocks and loaded through a range table, reducing the number of files without changing the Catalog UI. `catalogBlockSize` controls the approximate block size. Other entity types remain individually addressable files.
